...
namespace Tap_Taste
{
    public partial class Form4 : Form
    {
        ...
        // Constructeur par défaut : Utilisé pour l'AJOUT (appelé depuis btnadd_Click de Form3)
        public Form4()
        {
            InitializeComponent();
            isEditMode = false;
        ...
        // Le constructeur initial avec 4 arguments est supprimé car le constructeur par défaut est utilisé pour l'ajout.
        // public Form4(string id, string nom, string prix, string desc) { ... }

        // Constructeur pour la MODIFICATION (appelé depuis btnmodifier_Click de Form3)
        public Form4(string id, string nom, string prix, string desc, string idCat, string img)
        {
            ...
            idProduit = Convert.ToInt32(id); // <<< AJOUT IMPORTANT

            textBoxNom.Text = nom;
            textBoxPrix.Text = prix;
            textBoxDescription.Text = desc;

            // On stocke la catégorie pour la sélectionner dans Form4_Load (si la ComboBox est prête)
            this.Tag = idCat;
            ...
            using (SqlConnection con = new SqlConnection(conString))
            {
                try
                {
                    ...
                    if (!isEditMode)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@prix", prixProduit); // Utilisation du float parsé
